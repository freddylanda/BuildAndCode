...
public class Principal {
	...
	public static void main(String[] args) {
		...
		Pelicula pelicula2 = new Pelicula("Dark Knight", 2003, "Christopher Nolan", 9.0);	
		...
		do {
			...
			try {
				...
				case 1:
					...
					for(Pelicula p : imp.lista) {
						if(p.getTitulo().equalsIgnoreCase(titulo) || p.getAnio()==anio ||
								p.getDirector().equalsIgnoreCase(director) || p.getClasificacion()==clasificacion) {
							repetido = true;
							break;
						}
					}
					...
					if(repetido) {
						System.out.println("No se puede agregar, alguno de los atributos ya existe en la lista.\n");
					}else {
				...
				case 2: 
					...
					for(Pelicula p : imp.lista) {
						switch (opcion) {
						case 1:
							if(p.getTitulo().equalsIgnoreCase(valor)) {
								resultados.add(p);
							}
							break;
						case 2:
							try {
							int auxAnio = Integer.parseInt(valor);
							if(p.getAnio()==auxAnio)
								resultados.add(p);
						...
						case 4:
							try {
								int auxClasif = Integer.parseInt(valor);
								if(p.getClasificacion()==auxClasif)
									resultados.add(p);
					...
					if(resultados.isEmpty()){
						System.out.println("No se encuentraron coincidencias");
					}else {
						for(Pelicula p : resultados) {
							System.out.println("Pelicula [titulo=" + p.getTitulo() + ", año=" + p.getAnio() + 
									", director=" + p.getDirector() + ", clasificacion=" + p.getClasificacion() + "]");
				...
				case 4:
					...
					System.out.println("Indique el atributo a eliminar");
					System.out.println("1. TITULO");
					System.out.println("2. AÑO");
					System.out.println("3. DIRECTOR");
					System.out.println("4. CLASIFICACIÓN");
					...
					System.out.println("Ingrese el valor a Buscar");
					String val = datos.nextLine().trim();
						
					Iterator<Pelicula> i = imp.lista.iterator();
					...
					while(i.hasNext()) {
						...
						try {														
							switch(opc) {
							case 1:				
								if(p.getTitulo().equalsIgnoreCase(val))
									eliminar = true;
								break;
							case 2:
								try {
									int auxAnio = Integer.parseInt(val);
									if(p.getAnio()==auxAnio)
										eliminar = true;
								...
								break;
							case 3:
								if(p.getDirector().equalsIgnoreCase(val))
									eliminar = true;
								break;
							case 4:
								try {
									double auxClasif = Double.parseDouble(val);
									if(p.getClasificacion()==auxClasif)
										eliminar = true;
					...
					if(count>0) {
						...
						System.out.println("No se encontraron registros a eliminar.");
					}
					break;
				case 5:
